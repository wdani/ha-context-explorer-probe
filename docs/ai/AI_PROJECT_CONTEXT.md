...
The distribution-readiness starter adds minimal HACS custom repository metadata, provisional README logo/icon presentation, local integration brand assets, release/tag workflow guidance, a HACS custom repository test checklist, and installation/update documentation clarity. Live HACS testing confirmed custom repository acceptance, download availability, and README text rendering before the domain rename; the README logo now uses a raw GitHub URL because the earlier relative image did not render in HACS. Version `0.5.0` updates the future Brands path to `custom_integrations/ha_context_explorer/` and `https://brands.home-assistant.io/_/ha_context_explorer/icon.png`. A post-rename readiness pass adds `brand/icon@2x.png` and prepares `docs/brands/home-assistant-brands/custom_integrations/ha_context_explorer/icon.png` plus `icon@2x.png` for a future external Brands PR. This prepares for future HACS custom repository testing, Home Assistant 2026.3+ local custom integration brand discovery, and release-based updates, but it is not a default-store submission, executed Home Assistant Brands submission, release automation pass, guaranteed icon-display claim, or backward-compatible migration.
