...
The distribution-readiness starter keeps version `0.4.1` and adds only minimal HACS custom repository metadata, provisional README logo/icon presentation, local integration brand assets, release/tag workflow guidance, a HACS custom repository test checklist, and installation/update documentation clarity. Live HACS testing confirmed custom repository acceptance, download availability, and README text rendering; the README logo now uses a raw GitHub URL because the earlier relative image did not render in HACS. Home Assistant's own integration/repairs UI can show the local integration icon in the tested runtime. The HACS list/card icon remains unavailable because HACS/Home Assistant requests `https://brands.home-assistant.io/_/ha_context_explorer_probe/icon.png`, which currently returns the central Home Assistant Brands placeholder until a separate `home-assistant/brands` PR adds `custom_integrations/ha_context_explorer_probe/` assets. This prepares for future HACS custom repository testing, Home Assistant 2026.3+ local custom integration brand discovery, and release-based updates, but it is not a default-store submission, Home Assistant Brands submission, release automation pass, guaranteed icon-display claim, or runtime feature change.
